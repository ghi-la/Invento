...
export default function MembersClient({ initialMembers, initialInvitations }) {
  const { t } = useTranslation();
  const { warehouseId } = useParams();
  const { role: myRole } = useWarehouse();
  const { data, isLoading, mutate } = useSWR(`/api/warehouses/${warehouseId}/members`, {
    fallbackData: initialMembers ? { members: initialMembers } : undefined,
  });
  ...
  return (
    <Box sx={{ maxWidth: 640, mx: "auto" }}>
      ...
      <InvitationsSection warehouseId={warehouseId} assignable={assignable} initialInvitations={initialInvitations} />
    </Box>
  );
}

function InvitationsSection({ warehouseId, assignable, initialInvitations }) {
  const { t } = useTranslation();
  const { data, isLoading, mutate } = useSWR(`/api/warehouses/${warehouseId}/invitations`, {
    fallbackData: initialInvitations ? { invitations: initialInvitations } : undefined,
  });
